...
from .valid_tags import describe_tag, is_tag, has_arg
...
def read_next_int(in_str: str, start_pos: int) -> Tuple[Optional[int], int]:
...
def read_next_tag(in_str: str, start_pos: int) -> Tuple[str, Optional[int], int]:
    for test in range(3, 0, -1):
        end_pos = start_pos+test
        possible_tag: str = in_str[start_pos:end_pos]
        possible_arg: Optional[int] = None
        if not is_tag(possible_tag):
            continue
        if has_arg(possible_tag):
            possible_arg, end_pos = read_next_int(in_str, end_pos)
...
def parse_string(animation_string: str) -> List[Frame]:
    ...
    while pos < len(animation_string):
        ...
        if ch.isupper():
            frame_key = ord(ch) - 65
            frame_duration, pos = read_next_int(animation_string, pos + 1)
            ...
            tag_name, tag_value, pos = read_next_tag(animation_string, pos)
            current_tags[tag_name] = tag_value
        elif ch == "-":
            pos += 1
            frame_start = pos
        elif ch in ["u", "c", "p", "o", "z"]:
            pos += 1
        else:
